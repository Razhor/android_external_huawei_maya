...
#ifndef CONFIG_HUAWEI_ANDROID
#define CONFIG_HUAWEI_ANDROID
#endif
...
#ifdef CONFIG_HUAWEI_ANDROID
#define ANDROID_MOUNTED  "/proc/mounts"
#define HW_E4DEFRAG_TMPFILE	"/data/local/tmp/"
#define HW_E4DEFRAG_LOGDIR	"/data/log/defrag_log"
#define HW_E4DEFRAG_LOGFILE	"/data/log/defrag_log/e4defrag.log"
#define HW_E4DEFRAG_CONFILE	"/data/local/tmp/e4defrag.conf"

static unsigned int hw_scanned_count;
static unsigned int hw_succeed_count;
static unsigned int hw_failure_count;
static unsigned long hw_defrag_size;
static unsigned long hw_defrag_block;
static int hw_defrag_max_size;
static int hw_defrag_min_size;
static int hw_defrag_score;
static int log_enable;
static int hw_log_fd;
static sigset_t newmask, oldmask;
static int hw_e4defrag_init();
static int hw_e4defrag_exit();
static int hw_e4defrag_start();
static int hw_e4defrag_stop();
static void hw_block_sig();
static void hw_unblock_sig();
...
static int get_bytes(char *str)
{
	...
	if (!strncmp(endp, "KB", 2)) {
		value *= 1024;
	} else if (!strncmp(endp, "MB", 2)) {
		value *= 1024 * 1024;
	}
...
#ifdef CONFIG_HUAWEI_ANDROID
static __s64 file_size_uplimit = ((((__u64) 1) << 63) - 1);
static char file_exclude_postfix[1024] = "";

int check_exclude_postfix(const char *file)
{
	char *postfix;
	char *save = NULL;
	char postfix_copy[1024];

	strncpy(postfix_copy, file_exclude_postfix, sizeof(postfix_copy));
	postfix = strtok_r(postfix_copy, ",", &save);
	while (postfix != NULL) {
	    if (strlen(file) < strlen(postfix)) {
	        continue;
        }
	    if (strncmp(postfix, file + strlen(file) - strlen(postfix), strlen(postfix)) == 0) {
	        return 1;
        }
	    postfix = strtok_r(NULL, ",", &save);
	}
	return 0;
}
#endif
...
/* Read one mount table entry from STREAM.  Returns a pointer to storage
   ...
   check).  */
struct mntent *__getmntent_r (FILE *stream, struct mntent *mp, char *buffer, int bufsiz)
{
  char *cp;
  char *head;
...
struct mntent *hw_getmntent (FILE *stream)
{
...
{
	/* Refer to /etc/mtab */
	const char	*mtab = ANDROID_MOUNTED;
	FILE		*fp = NULL;
	struct mntent	*mnt;
	struct stat64	sb;
    ...
    if (getmntent_buffer == NULL) {
        perror("Couldn't malloc mnt ent buffer");
        return -1;
    }
	...
	while ((mnt = hw_getmntent(fp)) != NULL) {
		struct stat64 ms;
		/*
...
#ifdef CONFIG_HUAWEI_ANDROID
	if (buf->st_size > file_size_uplimit) {
		if (mode_flag & DETAIL) {
			PRINT_FILE_NAME(file);
			STATISTIC_ERR_MSG("File is bigger then size uplimit");
		}
		return 0;
	}

	if (check_exclude_postfix(file)) {
		if (mode_flag & DETAIL) {
			PRINT_FILE_NAME(file);
			STATISTIC_ERR_MSG("File postfix is excluded");
		}
		return 0;
	}
#endif
	...
	const struct stat64 *buf, struct fiemap_extent_list *ext_list_head)
{
	...
	do {
		...
		if (ret < 0) {
#ifdef CONFIG_HUAWEI_ANDROID
			if ((mode_flag & DETAIL) || (mode_flag & SILENCE)) {
#else
			if (mode_flag & DETAIL) {
#endif
				printf("\n");
			...
			struct FTW *ftwbuf EXT2FS_ATTR((unused)))
{
	...
	defraged_file_count++;
#ifdef CONFIG_HUAWEI_ANDROID
	if ((mode_flag & SILENCE) && log_enable) {
		hw_scanned_count++;
		if ((buf->st_size > hw_defrag_max_size) ||
				(buf->st_size < hw_defrag_min_size))
			return 0;
	}
#endif
	if (mode_flag & DETAIL) {
	...
	if (current_uid == ROOT_UID)
		...
		best = 1;
#ifdef CONFIG_HUAWEI_ANDROID
	if ((mode_flag & SILENCE) && current_uid == ROOT_UID) {
		float ratio;

		ratio = (float)(orig_physical_cnt - best) * 100 / blk_count;
		if (CALC_SCORE(ratio) < hw_defrag_score)
			goto out;
	}
#endif
...
#ifdef CONFIG_HUAWEI_ANDROID
	if (mode_flag & SILENCE) {
		int log_path_len = (int)strnlen(HW_E4DEFRAG_TMPFILE, PATH_MAX);

		sprintf(tmp_inode_name, "%s", HW_E4DEFRAG_TMPFILE);
		sprintf(tmp_inode_name + log_path_len, "%.*s.defrag",
				(int)strnlen(basename(file), PATH_MAX - log_path_len),
				basename(file));
	} else {
...
check_improvement:
	...
	if (file_frags_start <= best ||
		...
		succeed_cnt++;
#ifdef CONFIG_HUAWEI_ANDROID
		if ((mode_flag & SILENCE) && (orig_physical_cnt <= donor_physical_cnt))
			hw_failure_count++;
#endif
		if (file_frags_start != 1)
	...
	if (mode_flag & SILENCE) {
		hw_succeed_count++;
		hw_defrag_size += buf->st_size;
		hw_defrag_block += buf->st_blocks;
	}
#endif
...
#ifdef CONFIG_HUAWEI_ANDROID
static void hw_sig_quit(int signo)
{
	if (log_enable) {
		printf("^^^^^^^^^ force quit\n");
		hw_e4defrag_stop();
...
static void hw_block_sig()
{
	if (sigprocmask(SIG_BLOCK, &newmask, &oldmask) < 0)
		perror("fail to block SIG_QUIT");
}

static void hw_unblock_sig()
{
	if (sigprocmask(SIG_SETMASK, &oldmask, NULL) < 0)
		perror("fail to unblock SIG_QUIT");
}

static int read_configure()
{
	size_t len = 0;
	FILE *fcon;
	int read, option_count;
	char *buffer = NULL, *opt;
	...
	option_count = sizeof(hw_options) / sizeof(struct option_set);

	read = getline(&buffer, &len, fcon);
	while (read != -1) {
		int pos = 0, i = 0, opt_len;
		char *opt;
		...
		for (i = 0; i < option_count; i++) {
			opt = hw_options[i].option;
			opt_len = hw_options[i].len;

			if (!strncmp(opt, buffer + pos, opt_len)) {
				break;
			}
		}
		...
		switch(i) {
		case 0:
			hw_defrag_max_size = get_bytes(buffer + pos);
			if (hw_defrag_max_size < 0) {
				printf("Invalid configuration:\n %s", buffer);
				goto outclose;
			}
			break;
		case 1:
			hw_defrag_min_size = get_bytes(buffer + pos);
			if (hw_defrag_min_size < 0) {
				printf("Invalid configuration:\n %s", buffer);
				goto outclose;
			}
			break;
		case 2:
			hw_defrag_score = strtoul(buffer + pos, NULL, 10);
			if (hw_defrag_score < 0) {
				printf("Invalid configuration:\n %s", buffer);
				goto outclose;
			}
			break;
...
nextline:
	...
	if (hw_defrag_max_size < hw_defrag_min_size) {
		printf("Invalid file size:\n max_size = %d\n min_size = %d\n",
			hw_defrag_max_size, hw_defrag_min_size);
		goto outclose;
	}
...
static int hw_e4defrag_init()
{
	...
	if ((property_get("huawei.defrag.nolog", debug, NULL) != 0)
			&& (!strcmp(debug, "1"))) {
		log_enable = 0;
	}
	else {
	...
	if (log_enable) {
		if (access(HW_E4DEFRAG_LOGDIR, 0)) {
            char *cmd_path = NULL;
            char* cmd_argv[3];

            cmd_path="/system/bin/mkdir";
            cmd_argv[0]="mkdir";
            cmd_argv[1]=HW_E4DEFRAG_LOGDIR;
            cmd_argv[2]=NULL;
            exec_cmd(cmd_path, cmd_argv);
		}

		if (!access(HW_E4DEFRAG_LOGFILE, 0)) {
			struct stat buf;
			char *cmd_path = NULL;
			char* cmd_argv[3];

			stat(HW_E4DEFRAG_LOGFILE, &buf);
			if (buf.st_size > 1024 * 1024) {
                cmd_path="/system/bin/gzip";
                cmd_argv[0]="gzip";
                cmd_argv[1]=HW_E4DEFRAG_LOGFILE;
                cmd_argv[2]=NULL;
                exec_cmd(cmd_path, cmd_argv);
			}
		}

		hw_log_fd = open(HW_E4DEFRAG_LOGFILE, O_RDWR | O_CREAT | O_APPEND, S_IRWXU);
		if (hw_log_fd < 0) {
		...
		printf("######### e4defrag start at: %s", ctime(&start_time));
	}

	if (!access(HW_E4DEFRAG_CONFILE, F_OK | R_OK)) {
		err = read_configure();
		if (err)
			goto out;
	} else {
		...
		hw_defrag_min_size = 32 * 1024;
		hw_defrag_max_size = 100 * 1024 * 1024;
		hw_defrag_score = 30;
	}
	...
	if ((signal(SIGQUIT, hw_sig_quit)) == SIG_ERR) {
		if (log_enable)
			perror("fail to catch SIGQUIT");
		err = -EINVAL;
	}
...
static int hw_e4defrag_exit()
{
	...
	if (log_enable) {
		...
		printf("######### e4defrag finish at: %s", ctime(&finish_time));
	}

	return err;
}

static int hw_e4defrag_start(char *file)
{
	int err = 0;
...
static int hw_e4defrag_stop()
{
	...
	if (log_enable) {
		printf("scanned file count = %u\n", hw_scanned_count);
		printf("defrag file count  = %u\n", hw_succeed_count);
		printf("defrag fail count  = %u\n", hw_failure_count);
		printf("total defrag size  = %lu\n", hw_defrag_size);
		printf("total defrag block = %lu\n", hw_defrag_block);
		printf("\n");
	}
...
int main(int argc, char *argv[])
{
...
#ifdef CONFIG_HUAWEI_ANDROID
	while ((opt = getopt(argc, argv, "vcds:p:")) != EOF) {
#else
	while ((opt = getopt(argc, argv, "vcs:p:")) != EOF) {
#endif
		switch (opt) {
		case 'v':
			mode_flag |= DETAIL;
		...
		case 's':
            if(optarg == NULL) {
                printf("Option argument -%c need an operand!\n", opt);
                goto out;
            }
			file_size_uplimit = atoll(optarg) * 1024;
			break;
		case 'p':
            if(optarg == NULL) {
                printf("Option argument -%c need an operand!\n", opt);
			...
			strncpy(file_exclude_postfix, optarg, sizeof(file_exclude_postfix));
			file_exclude_postfix[sizeof(file_exclude_postfix)-1] = 0;
			break;
#endif
		default:
			goto out;
	...
	current_uid = getuid();
#ifdef CONFIG_HUAWEI_ANDROID
	if ((mode_flag & SILENCE) && hw_e4defrag_init())
		exit(1);
#endif
	...
	for (i = optind; i < argc; i++) {
...
#endif
#ifdef CONFIG_HUAWEI_ANDROID
		if ((mode_flag & SILENCE) && hw_e4defrag_start(argv[i]))
			exit(1);
#endif
		...
		case DIRNAME:
#ifdef CONFIG_HUAWEI_ANDROID
			if (!(mode_flag & STATISTIC) && !(mode_flag & SILENCE))
#else
			if (!(mode_flag & STATISTIC))
#endif
...
#ifdef CONFIG_HUAWEI_ANDROID
		if ((mode_flag & SILENCE) && hw_e4defrag_stop())
			exit(1);
	}

	if (mode_flag & SILENCE)
		return hw_e4defrag_exit();
#else
	}
#endif
	if (success_flag)
